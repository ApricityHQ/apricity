...
\normalsize
\begin{itemize}
    \item Traditional startup viability assessments are slow, expensive, and often rely on external consultants.
    \item Manual evaluation processes can take weeks, risking delayed pivots and missed market opportunities.
    \item Our solution introduces AI-powered agents that simulate expert consultants—providing fast, cost-effective assessments.
    \item These agents deliver real-time, multi-perspective feedback, helping founders refine their ideas quickly and efficiently.
\end{itemize}
\end{frame}
...
\begin{itemize}
    \item \textbf{AI-Powered Evaluation}: Leverages advanced language models to simulate expert-level analysis across multiple domains.
    \item \textbf{Multi-Agent Collaboration}: Agents work together, combining insights for holistic startup assessments.
    \item \textbf{Modular System Design}: Easily adaptable—agents can be added, removed, or customized based on evaluation needs.
    \item \textbf{Domain-Specific Expertise}: Each agent is fine-tuned for a specific area like finance, tech, product, or marketing.
...
\begin{frame}{Features}
\begin{itemize}
\item \textbf{Real-Time Feedback}: Founders receive actionable insights in minutes rather than weeks.
    \item \textbf{Retrieval-Augmented Generation (RAG)}: Ensures that responses are grounded in the latest, relevant information from trusted sources.
    \item \textbf{Scalable and Cost-Effective}: Eliminates the need for expensive consulting while maintaining expert-level quality.
    \item \textbf{Interactive Reports}: Provides structured summaries, risk flags, and improvement suggestions tailored to the startup's context.
...
In the fast moving world of startups, many founders face uncertainty about whether their ideas will succeed, often relying on intuition or costly expert consultations for guidance. Existing evaluation methods are either too subjective or depend heavily on limited financial data, leaving early stage startups without a reliable way to understand their real potential.
...
\begin{itemize}
    \item Evaluates early-stage startups using analytical agents for market, product, and founder analysis
    \item Collects structured/unstructured data and external market insights
    \item Produces clear, qualitative assessments with decisions explained in natural language
    \item Ensures transparent, adaptable analysis for faster, confident investment decisions
\end{itemize}
\end{frame}
...
\begin{frame}
\frametitle{Input Module - Purpose}
\begin{itemize}
    \item Collects and organizes startup-related input data from the user.
    \item Establishes the foundational dataset for further AI-based evaluation.
\end{itemize}
\end{frame}

\begin{frame}
\frametitle{Input Module - Functions}
\begin{itemize}
    \item Accepts user inputs through a web form and file uploads.
    \item Extracts key details like industry domain, business model, funding stage, and target market.
    \item Performs basic text preprocessing to ensure consistency.
\end{itemize}
\end{frame}

\begin{frame}
\frametitle{Input Module - Input/Output}
\begin{itemize}
    \item \textbf{Input:} User-submitted form data and startup pitch/report documents.
    \item \textbf{Output:} Structured startup profile prepared for enrichment.
\end{itemize}
\end{frame}

\begin{frame}
\frametitle{Input Module - Libraries Used}
\begin{itemize}
    \item \textbf{React:} For creating a dynamic web-based frontend for data and file inputs.
    \item \textbf{Chunkr:} For splitting uploaded documents into meaningful chunks to enable structured data extraction.
\end{itemize}
\end{frame}

%---------------------- Web Crawler Module ----------------------
\subsection{Web Crawler Module}

\begin{frame}
\frametitle{Web Crawler Module - Purpose}
\begin{itemize}
    \item Completes missing information by retrieving data from the web.
    \item Ensures a more accurate and reliable startup evaluation.
\end{itemize}
\end{frame}

\begin{frame}
\frametitle{Web Crawler Module - Functions}
\begin{itemize}
    \item Scrapes trusted online sources like Crunchbase, LinkedIn, and news articles.
    \item Gathers additional data such as founder background, funding history, and competitors.
    \item Merges web findings with user-provided startup data.
\end{itemize}
\end{frame}

\begin{frame}
\frametitle{Web Crawler Module - Input/Output}
\begin{itemize}
    \item \textbf{Input:} Partial startup context from Input Module.
    \item \textbf{Output:} Enriched dataset with verified and up-to-date information.
\end{itemize}
\end{frame}

\begin{frame}
\frametitle{Web Crawler Module - Libraries Used}
\begin{itemize}
    \item \textbf{Perplexity API:} For fetching the most relevant web-based information.
    \item \textbf{Exa.ai API:} For AI-powered search and contextual web data retrieval.
\end{itemize}
\end{frame}
...
\begin{frame}
\frametitle{Agent Module - Purpose}
\begin{itemize}
    \item Conducts multi-domain evaluation using specialized AI agents.
    \item Mimics expert reasoning to assess startup potential.
\end{itemize}
\end{frame}

\begin{frame}
\frametitle{Agent Module - Functions}
\begin{itemize}
    \item Runs domain-based analysis through agents:
    \begin{itemize}
        \item Financial Analyst – revenue model and funding sustainability
        \item CTO – technology feasibility and scalability
        \item Marketing Specialist – market reach and branding
        \item Sales Manager – customer acquisition potential
        \item Product Analyst – product-market fit
        \item Venture Capitalist – investment viability
    \end{itemize}
    \item Generates structured insights and reasoning for each domain.
\end{itemize}
\end{frame}

\begin{frame}
\frametitle{Agent Module - Input/Output}
\begin{itemize}
    \item \textbf{Input:} Enriched startup data from Web Crawler Module.
    \item \textbf{Output:} Agent-wise evaluations with justification summaries.
\end{itemize}
\end{frame}

\begin{frame}
\frametitle{Agent Module - Libraries Used}
\begin{itemize}
    \item \textbf{OpenAI API:} For generating domain-specific insights using LLMs.
    \item \textbf{Gemini API:} Alternative LLM source for reasoning diversity.
    \item \textbf{LangChain:} For orchestrating multi-agent workflows and reasoning chains.
\end{itemize}
\end{frame}
...
\begin{frame}
\frametitle{Report Generation Module - Purpose}
\begin{itemize}
    \item Consolidates expert agent outputs into a readable final report.
    \item Provides actionable and comprehensive startup evaluation.
\end{itemize}
\end{frame}

\begin{frame}
\frametitle{Report Generation Module - Functions}
\begin{itemize}
    \item Combines all agent findings into a unified evaluation.
    \item Generates success score using a weighted scoring model.
    \item Summarizes strengths, weaknesses, opportunities, and risks (SWOT).
\end{itemize}
\end{frame}

\begin{frame}
\frametitle{Report Generation Module - Input/Output}
\begin{itemize}
    \item \textbf{Input:} AI agent evaluations and reasoning insights.
    \item \textbf{Output:} Final structured evaluation report for users and investors.
\end{itemize}
\end{frame}

\begin{frame}
\frametitle{Report Generation Module - Libraries Used}
\begin{itemize}
    \item \textbf{ReportLab:} For programmatic PDF report generation.
    \item \textbf{Fpdf:} For lightweight and fast PDF creation.
    \item \textbf{WeasyPrint:} For HTML-to-PDF report rendering with visual design.
\end{itemize}
\end{frame}
